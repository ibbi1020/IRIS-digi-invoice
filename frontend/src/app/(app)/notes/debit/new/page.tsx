...
export default function NewDebitNotePage() {
  ...
  const methods = useForm<InvoiceDocumentFormData>({
    ...
    defaultValues: createDefaultInvoiceFormData('DEBIT_NOTE'),
  });
  ...
  React.useEffect(() => {
    ...
    const checkDuplicate = async () => {
      ...
      if (used) {
        if (status === 'SUCCESS') {
          setBlockReason(
            `Reference "${invoiceRefNo}" has already been successfully submitted. Please use a different reference number.`
          );
        } else if (status === 'UNKNOWN') {
          setBlockReason(
            `Reference "${invoiceRefNo}" has an unknown submission status. Please use a different reference number.`
          );
        } else {
          setBlockReason(null);
        }
  ...
  const validateReference = async (refNo: string) => {
    if (!sellerIdentity) return { valid: false, reason: 'Seller identity not configured' };
    const repo = getStorageRepository();
    return repo.isReferencedInvoiceValid(refNo, sellerIdentity.sellerNTNCNIC);
  };
  ...
  const onSubmit = async (data: InvoiceDocumentFormData) => {
    ...
    // Validate referenced invoice
    const refValidation = await validateReference(data.referencedInvoiceRefNo || '');
    if (!refValidation.valid) {
      setError('referencedInvoiceRefNo', { type: 'validate', message: refValidation.reason });
      return;
    }
    ...
    const document: InvoiceDocument = {
      ...
      referencedInvoiceRefNo: data.referencedInvoiceRefNo,
    ...
    const handleAttemptComplete = async (result: SubmitAttemptResult) => {
      const attempt: AttemptEntry = {
        ...
        responseSummary: result.success ? 'Debit note submitted successfully' : result.error?.message,
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="text-muted-foreground">
            Issue a debit note against an existing sales invoice
          </p>
        </div>
        ...
        <Alert variant="warning">
          ...
          <AlertDescription>
            Please configure your seller identity in Settings before creating debit notes.
          </AlertDescription>
        </Alert>
    ...
    <div className="space-y-6">
      ...
      {submitSuccess && (
        <Alert variant="success">
          ...
          <AlertTitle>Debit Note Submitted Successfully</AlertTitle>
          <AlertDescription>
            Your debit note has been submitted to FBR. Redirecting to dashboard...
          </AlertDescription>
        </Alert>
      )}
      ...
      <FormProvider {...methods}>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          <InvoiceHeaderSection
            ...
            onValidateReference={validateReference}
          />
